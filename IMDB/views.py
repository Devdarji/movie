...
def home(request):
    ...
    for i in range(len(movies_urls)):
        ...
        movie_soup = BeautifulSoup(
            movie_site.content, 'lxml', from_encoding="iso-8859-1")
        ...
        movies_release_date.append(movie_soup.find('div', {"class": "hWHMKr"}).find(
    ...
    for i in range(len(movies_urls)):
        movie = MovieData(
            title=movies_title[i], rating=movies_rating[i], release_date=movies_release_date[i], duration=movies_duration[i], description=movies_description[i])
        print(movie)
        movie.save()
